/**
 * The Commissioner-only `/roster-move` route: a three-step gated `load` and
 * one `record` action that commits the Move (Story 7.7, FR-41).
 *
 ...
 * still typed and the Move still refused with JavaScript switched off. That
 ...
 * all re-derived inside `recordRosterMove`'s transaction, under the global
 * lock, from the log and the tables that transaction itself read. The sheet
 * is rendered from the SAME `evaluateMove` a moment earlier, so the two
...
import { describeMoveAmount, rosterMoveRefusalDetail } from '$lib/core/rules/roster-move.ts';
import type { MovingTeam } from '$lib/core/rules/roster-move.ts';
import {
	ROSTER_MOVE_COMMIT_LABEL,
	reasonSheetView,
	rosterMoveActSentence,
	rosterMoveReasonRows
...
import {
	...
	previewRosterMove,
	recordRosterMove
} from '$lib/server/roster-move.ts';
import type { RosterMoveRejection } from '$lib/server/roster-move.ts';
...
function pickerRowsFor(team: MovingTeam) {
	return {
		teamId: team.teamId,
		...
		players: team.rows
			...
			.map((row) => ({
				...
				// `describeMoveAmount`, for the sheet's reason: an imported Cap Hit
...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	if (from === '' || to === '' || from === to) {
		return {
			phase: locals.phase,
			step: 'teams' as const,
			teams: await loadRosterMoveTeams(writeGateway()),
	...
	const preview = await previewRosterMove(writeGateway(), {
		sendingTeamId: from,
		receivingTeamId: to,
		sendingPlayerIds,
		receivingPlayerIds,
	...
	if (confirming) {
		if (preview.outcome.kind === 'permitted') {
			...
			// the browser silently drops — which would post a Move naming no
			...
			return {
				...
				sheet: reasonSheetView({
					act: rosterMoveActSentence(delta),
					commitLabel: ROSTER_MOVE_COMMIT_LABEL,
					rows: rosterMoveReasonRows(delta),
		...
		return {
			...
			refusal: {
				...
				detail: rosterMoveRefusalDetail(preview.outcome.refusal, preview.outcome.gates)
			}
		};
	}
...
export const actions: Actions = {
	record: async ({ request, locals, url }) => {
		...
		if (actor === null) {
			...
			return fail(400, {
				notice: 'This account is not bound to a Team, so there is no actor to record the Move under.'
			});
		}
		...
		if (outcome.kind === 'rejected') {
			const rejection = outcome.reason as RosterMoveRejection | undefined;
			return fail(409, { notice: rejection?.detail ?? 'The Move was refused.' });
		}

		const appended = outcome.events[0];
		return {
			notice:
				'The Roster Move is recorded. Both Teams’ Cap Space, Roster Count and Slot ' +
