...
>
> Three consequences a reader should not have to discover. First, **a Trade can be refused because of the Team giving players up** — a freed Slot costs $1,000,000 of Roster Reserve against a Cap Hit that may be worth less. Second, **a Drop moves Maximum Bid in either direction**: down from Active/Bench, up from a Minor League Slot via Minors Exposure. Third, **a Trade or Drop is refused, never cancelled around** — FR-40's cancellation trigger is still an Auction Close and nothing else. Examples **36–43** make all of it executable.
...
- A divergence is classified as one of three kinds:
  ...
  - **Unpaired departure** — a Player the app holds is on no Fantrax roster. This is a **Drop**, and it is raised as a proposed Drop under FR-43. It is a distinct act from a Trade: one Team, one direction, and no cap relief except in the 2RK case FR-43 defines.
