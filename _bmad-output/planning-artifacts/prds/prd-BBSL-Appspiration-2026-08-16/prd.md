...
- A Contention whose Contender list is **empty at expiry** — every Contender having been cancelled or retracted — closes with no winner, and the Player returns to the Free Agent pool unclaimed. The empty list and the reason are recorded exactly as a drawn one would be.
...
*Added 2026-09-16 with the retraction window (FR-15). Examples 1 – 46 are unchanged. Note that 47 – 54 turn on **seconds**: the window is ninety of them, and an example written in whole minutes would not fit inside it:*
