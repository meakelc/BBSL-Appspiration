...
>
> Three consequences a reader should not have to discover. First, **a Trade can be refused because of the Team giving players up** — a freed Slot costs $1,000,000 of Roster Reserve against a Cap Hit that may be worth less. Second, **a Drop moves Maximum Bid in either direction**: down from Active/Bench, up from a Minor League Slot via Minors Exposure. *(Still true, and now for one reason fewer — from 2026-09-16 an Active/Bench Drop moves it in one direction only.)* Third, **a Trade or Drop is refused, never cancelled around** — FR-40's cancellation trigger is still an Auction Close and nothing else. Examples **36–43** make all of it executable.
>
...
>
> **One consequence a reader should not have to discover.** **An Active/Bench Drop can no longer *raise* a Team's Maximum Bid under any circumstance.** Nothing returns to Cap Space on any release, so only three directions remain, and FR-43's existing table already states all three: Active/Bench **lowers** it by the $1,000,000 reserve on the freed hole, Injury Reserve leaves it **unchanged**, Minor League **raises** it via Minors Exposure. §10 example **41** exists only to demonstrate the exception and is **retired** rather than deleted.
>
...
**Consequences (testable):**
- For a Player who is not Minor League Eligible: `Maximum Bid = Available Cap Space − Roster Reserve`, where `Available Cap Space = Cap Space − Committed Bids` and `Roster Reserve = $1,000,000 × max(0, 12 − (Roster Count + Reserve Additions))`. *(Read **Reserve Additions**, not Projected Active/Bench Additions — the two split on 2026-09-18 and differ by a Team's held Minimum-Bid Contention entries. See the Glossary.)*
...
- **Cap Hit follows placement** (FR-21, FR-35). A Contract promoted into a Minor League Slot charges **$0**; one demoted to Active/Bench charges its **full** amount. Its value never changes and no expression reads one from the other.
- **Minors Exposure recomputes** across every eligible Auction the Team leads, and **Roster Count moves with Active/Bench occupancy.** A Move is therefore a Maximum Bid lever in **both directions**, and at least one of them is counterintuitive: *demoting* a stash to Active/Bench frees a Minor League Slot, which can **raise** Maximum Bid by reducing Minors Exposure, while simultaneously **lowering** Cap Space by charging the full Cap Hit. §10 example 45 is that case, and it is the FR-44 analogue of §10 example 40.
...
- Entries cover every Nomination, Bid, **Bid Retraction (FR-15)**, **Bid Cancellation and restoration (FR-40)**, Auction Close, Randomizer draw with seed, Commissioner override, import, and export. *(The first was added 2026-09-16; the second was an omission from the 2026-09-08 amendment, corrected the same day.)*
...
- **Slot Placement is re-evaluated for every moved Player against the receiving Team's occupancy** at the instant of the Trade, under FR-21's ordinary rule: a Minor League Eligible Player takes a Free Minor League Slot if one exists, and an Active/Bench Slot otherwise.
...
- The sending Team's freed Minor League Slot raises its **M**, which recomputes **Minors Exposure** across every eligible Auction it still leads — so a Trade can change a third Auction's Maximum Bid for a Team that was not party to it.
...
- **A Minor League Drop still leaves nothing behind, for the reason the table gives:** it was charging **$0**. That is placement arithmetic (FR-35), not relief, and it is the only route by which a Drop returns anything.
...
- Bidding with continuous Maximum Bid computation, server-side rejection of illegal bids, conservative commitment on Minor League Eligible players, Roster Capacity enforcement, and the **ninety-second** retraction window (FR-11 – FR-15, FR-35, FR-37).
...
45. **The Move that buys bidding power by spending cap.** Team L holds two of its three Minor League Slots (`M = 1`), has Roster Count 10 and Cap Space $20,000,000, and leads two **eligible** Auctions at $12,000,000 and $4,000,000. Today: `N = 2, M = 1`, Overflow Count 1, **Minors Exposure $12,000,000** (the larger), Available Cap Space $8,000,000; Projected Active/Bench Additions is `1` for the Bid being placed **plus `1` of Active/Bench Overflow** — one of the two eligible leads has no free Minor League Slot to land in (FR-37, §10 example 25) — so Roster Reserve is `$1,000,000 × max(0, 12 − 12) = $0`, and **Maximum Bid is $8,000,000**. It now **demotes** one stash — full value $2,000,000 — to an Active/Bench Slot. Minor League occupancy falls to 1, so `M = 2`; `N = 2 ≤ M = 2`, Overflow Count falls to **0** and **Minors Exposure falls to $0**. But the demoted Contract now charges, so **Cap Space falls to $18,000,000** — Available Cap Space is $18,000,000. Roster Count rises to 11, so Roster Reserve becomes `$1,000,000 × max(0, 12 − 12) = $0`, and **Maximum Bid is $18,000,000**. The Team **spent $2,000,000 of Cap Space and gained $10,000,000 of Maximum Bid.** *(Corrected 2026-09-12 by Story 7.11. The before figure previously read $7,000,000 and the gain $11,000,000, omitting the Active/Bench Overflow term from Projected Active/Bench Additions in the before state only — the after state has no overflow, so that figure was already right. Both self-consistent readings of the arithmetic put the gain at $10,000,000: the roster act's own gate, which projects no Bid, moves $7,000,000 → $17,000,000, and the Manager-facing Maximum Bid, which counts the Bid being placed, moves $8,000,000 → $18,000,000. Nothing the example exists to demonstrate changes.)* Every intuition says putting a player on the active roster costs you money; here it buys bidding power, because an empty Minor League Slot is what Minors Exposure reserves against. **This is the mirror of example 40**, and the reason FR-44's sheet must state which way Maximum Bid actually moved, in words, before commit.
