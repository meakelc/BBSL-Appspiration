...
Updated 2026-09-17: **no capability added — two amended, and one of them had been carrying an inverted rule.** CAP-3's Slot release was still stated as the PRD wrote it before **2026-09-14**: released at the Close of the nominated Player, whoever won. FR-9 was amended that day so that a Slot is released by **winning** a Player and by nothing else, and the spec never absorbed it — three days in which this contract and the shipped core disagreed about a rule every nomination gate reads. Corrected, with the consequence stated: a Team that never wins **nominates once for the whole auction**, and a Slot claim **outlives its board seat**. CAP-11 follows from it — a Close mentions the **winner alone** and states the Slot release on that same line, the **nominator is no longer mentioned** on a Close it did not win, and **`slot_release` is retired**, which leaves this league with **no mutable notification category at all**. FR-10's 24-hour notification went with it (removed from scope 2026-09-04); the 24-hour **flag** stays. Mappings are unchanged — CAP-3 still realizes FR-7 – FR-10 and CAP-11 still realizes FR-26 – FR-27 — so this pass moves no row in the table below. Driven by PRD ninth pass and commit `e5b9cbc`.
