...
Code review 2026-08-31 — 4 layers (Blind Hunter, Edge Case Hunter, Verification Gap, Acceptance Auditor/opus). 24 raw findings, 17 unique after merge: 1 decision-needed, 6 patch, 2 deferred, 8 dismissed. `npm test` 1975 pass / 1 pre-existing `SUPABASE_DB_URL` failure, `npm run check` 0/0, `check:purity` clean, `supabase/migrations/` and all read-only zones untouched.

- [x] [Review][Decision] The contracts fold and the two reducers that fold the same `AuctionClosed` disagree about what a malformed payload is — `nominationsReducer` and `auctionsReducer` both gate on `readClosedPlayerId` (`projection/nominations.ts:210`), which requires only a non-empty `fantraxPlayerId`. `contractsReducer`'s `readPayload` (`projection/contracts.ts`) additionally rejects a missing `teamId`, an unknown `placement`, and unparseable or negative money. So a payload carrying a valid Player id but a corrupt `teamId`/`placement`/amount releases the board seat AND removes the Auction while recording NO contract: the won Player is in no auction, no nomination, no contract and no `team_rosters` row, silently returning to the nominatable pool while the winning Team's Cap Space and Roster Count never move. The I/O matrix row "Malformed close payload" asserts "the other two reducers already skip it too" — true only when `fantraxPlayerId` is the missing field. Same decision covers the un-cross-checked fields `readPayload` does accept: an `active_bench` row whose `capHit` disagrees with `winningAmount` is charged as persisted (a `minor_league` one is neutralised by `computeCapSpace` re-zeroing it, which `contracts.ts` documents as deliberate). Not reachable from `decideClose`, which always emits a well-formed payload — this is an insert-only-log hardening question. Three defensible answers, with different consequences: (a) have all three reducers share one reader so a payload that fails contract validation leaves the Auction alive and visible; (b) loosen `readPayload` to repair rather than reject, contradicting its documented "a price cannot be guessed" reasoning; (c) accept the asymmetry and correct the spec's claim.
- [x] [Review][Patch] Example 17 re-registers Example 16's entire suite, running 8 cases twice per full run [tests/examples/example-17-minors-overflow.test.ts] — it imports `CONTRACTS_AFTER_16`, `TEAM_M_IMPORTED`, `appendedClose`, `auctionWonAt`, `minorLeagueOccupiedIn`, `nominationOf`, `rosterCountOf` from `./example-16-minors-placement.test.ts`, whose module-scope `describe` therefore executes again during 17's collection. Measured: 16 alone reports 8 tests, 17 alone reports 15 (its own 7 plus 16's 8), both together 23. Failure attribution for example 16 is duplicated across two files and the suite total is inflated. The AD-11 chaining requirement is genuinely met (`minorLeagueOccupiedIn(ROWS_AFTER_16)` is derived, not literal) — only the coupling mechanism is wrong. Fix: extract the shared fixtures into a non-`.test.ts` module.
- [x] [Review][Patch] A test names the shell/core winner-agreement property but asserts a tautology [tests/core/close.test.ts:199] — `expect(closedWinnerFor(auction, null)).toEqual(closedWinnerFor(auction, null))` calls one pure function twice with one argument; any pure function satisfies it. It exercises neither `server/close.ts` nor `decideClose`, so the Design Notes' "the shell cannot arrive at a different winner than the core does" is asserted nowhere.
- [x] [Review][Patch] `AT_EXPIRY`'s comment contradicts its own value [tests/core/close.test.ts:38] — the doc comment reads "One millisecond after the close" but line 39 assigns `AT_EXPIRY = CLOSES_AT`. `hasExpired` is `current >= close`, so the earliest legal instant IS `closesAt`; the constant's name and the test below it are correct, only the comment's stated offset is false. Found independently by all three recall layers.
- [x] [Review][Patch] An assertion tests an imported constant rather than the close [tests/server/close.test.ts:363] — `expect(SALARY_CAP).toBeGreaterThan(0)` sits under a comment claiming "the roster read still happened and still found the full Cap", but never asserts the Cap Space the close actually found.
- [x] [Review][Patch] `AuctionContract`'s docstring overclaims completeness while dropping `contention` [src/lib/core/projection/contracts.ts] — it promises "everything a downstream reading needs without re-folding the Auction it came from", but omits `contention`, which `AuctionClosedPayload` does carry and `readPayload` never reads. A consumer distinguishing a lottery win from a standard one would have to re-fold the Auction. The Code Map does not list `contention` among the fold's fields, so the code matches the spec and the docstring is what should narrow.
- [x] [Review][Patch] `auctionContracts()` is exported with no production caller [src/lib/core/projection/contracts.ts] — its only consumers are `tests/projection-contracts.test.ts`, and the Code Map's enumeration of this module's exports does not include it. Speculative surface in a story that argues carefully elsewhere for building only what a caller needs.
- [x] [Review][Defer] `closedWinnerFor` trusts a `ClosedWinner`'s `teamId`/`teamName`/`managerId` unchecked [src/lib/core/rules/close.ts] — an empty or malformed drawn winner would append an `AuctionClosed` naming an invalid Team or Manager against `not null` FK columns. No producer exists: Story 3.6 owns the drawer, and `closeAuction` passes `null` today. Deferred to 3.6, which should validate at the point it constructs the winner.
- [x] [Review][Defer] The spec's `git diff --stat` verification command does not cover the files the story authorises [spec Verification section] — it guards only `src/routes/`, `src/lib/components/`, `src/lib/shell/` and `rules/bidding.ts`, so an inadvertent edit to `core/types.ts` or `rules/roster-import.ts` would pass it silently. Both verified untouched here by hand, so nothing was breached; deferred as a note for future stories' verification sections.

**Resolution (2026-08-31).** Decision taken: **option (a)** — one shared reader. `projection/nominations.ts` now declares `readClosedFacts`, the single definition of a well-formed `AuctionClosed`, and `readClosedPlayerId` is a thin reading of it, so `nominationsReducer`, `auctionsReducer` and `contractsReducer` accept and skip exactly the same payloads. A close that fails validation now leaves the Auction standing and visible instead of dropping it while recording no contract. `SlotPlacement` and `SLOT_PLACEMENTS` moved to `core/types.ts` beside `RosterSlotKind` — the one edit to that file the Code Map pre-authorised, taken because `nominations.ts` sits upstream of `contracts.ts` in the import graph and the union has to be visible to both. `contracts.ts` re-exports `SlotPlacement`, so no call site moved.

All 6 patch findings applied. Test fixtures for §10 example 16 moved to `tests/fixtures/example-16-minors-placement.ts` (a plain module, no `describe`), ending the duplicate suite registration — example 17 alone now reports 7 tests rather than 15. `tests/fixtures/closed-event.ts` gives every suite one well-formed synthetic close, since a minimal `{ fantraxPlayerId }` payload is no longer a close.

The 10 new cases in `tests/projection-contracts.test.ts` were verified to discriminate: run against the previous weak reader, 8 fail and the 2 that pass are the two that should pass either way.

Final: `npm test` 1978 passed / 1 pre-existing `SUPABASE_DB_URL` failure (baseline 1975/1; net +3 = 10 new − 8 de-duplicated + 1 new lottery-agreement case). `npm run check` 0 errors 0 warnings. `check:purity` clean. `supabase/migrations/` and every read-only zone unchanged; `AUCTION_CLOSED_EVENT` still declared exactly once.
