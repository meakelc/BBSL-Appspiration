...
=== UNTRACKED FILES (full contents) ===

--- scripts/bmad-cost-report.py ---
#!/usr/bin/env python3
...
for f in "${FILES[@]}"; do
  ...
  # including the diff. Pipe it as stdin context; the -p argument is only the
  # instruction to obey it. Read is allowed because two of the three layers
  # point at an instruction file on disk.
  if ! cat "$f" | claude -p 'The provided content is a complete review instruction, including the content to review. Follow it exactly as written. Return only the review result.' \
      --model "$MODEL" \
      --allowedTools 'Read' \
      --max-turns 6 \
      --output-format json \
...
try:
    d = json.loads(pathlib.Path(raw).read_text(encoding="utf-8", errors="replace"))
...
if bad:
    print(f"   !! NOT WRITTEN — subtype={d.get('subtype')} "
          f"api_error_status={err} terminal_reason={term}")
